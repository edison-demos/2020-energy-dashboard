<template lang="pug">
  .mt-2
    b-row
      b-col
        h2.s-name.text-center 使用分析 -{{ title }}統計分析
    b-row.mt-3( align-v="end")
      ...
      b-col(cols=1)
        ...
        //- b-button(@click="startAnalysis" variant="outline-primary") 輸出
    b-row.mt-3
      b-col
          ...
          b-card.card
            b-row(align-h="end")
              b-dropdown(text="顯示項目" variant="warning" class="m-2")
                b-dropdown-form
                .m-dropdown
                    div(v-for="(cl, index) in analysisOptions" :key="index")
    ...
    b-row.legend(v-if="!lockRegionButton")
      b-col(cols=2 v-for="(cl, index) in analysisOptions" :key="index" v-if="!disables.includes(index)")
...
export default {
  data () {
    return {
      colors,
      analysisOptions: analysisOptions.map(text => ({
        name: text,
        checked: true,
      })),
  ...
  computed: {
    ...
    disables () {
      return this.analysisOptions.map((o, index) => o.checked ? OPTION_DISABLE : index).filter(v => v < OPTION_DISABLE)
    },
  },
  methods: {
    async startAnalysis () {
